<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.title" placeholder="请输入设备类别名称" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    >
      <el-table-column label="设备类别id" prop="id" sortable="custom" align="center" width="100" :class-name="getSortClass('id')">
        <template slot-scope="{row}">
          <span>{{ row.machineTypeId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="设备类别code" width="110" align="center">
        <template slot-scope="{row}">
          <span>{{ row.machineTypeCode }}</span>
        </template>
      </el-table-column>
      <el-table-column label="设备类别名称" width="110" align="center">
        <template slot-scope="{row}">
          <span class="link-type" @click="handleUpdate(row)">{{ row.machineTypeName }}</span>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="120px" style="width: 400px; margin-left:50px;">
        <el-form-item label="设备类别code" prop="machineTypeCode">
          <el-input v-model="temp.machineTypeCode" type="text" placeholder="请输入设备类别code" />
        </el-form-item>
        <el-form-item label="设备类别name" prop="machineTypeName">
          <el-input v-model="temp.machineTypeName" type="text" placeholder="请输入设备类别name" />
        </el-form-item>
...
import { fetchList, fetchPv, createDeviceType, updateDeviceType, updateUseFlag } from '@/api/deviceType'
import waves from '@/directive/waves' // waves directive
import { parseTime } from '@/utils'
import Pagination from '@/components/Pagination' // secondary package based on el-pagination

const machineTypeTypeOptions = []
...
const calendarTypeKeyValue = machineTypeTypeOptions.reduce((acc, cur) => {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        machineTypeCode: [{ required: true, message: '请填写设备类别code', trigger: 'blur' }],
        machineTypeName: [{ required: true, message: '请填写设备类别name', trigger: 'blur' }],
  ...
  methods: {
    ...
    /*     getMachineTypes() {
      fetchMachineTypeList().then(response => {
        ...
        this.machineTypeTypeOptions = response.data
        console.log('tag', this.machineTypeTypeOptions)
      })
    }, */
    // 立即刷新数据列表
    refreshList() {
    ...
    handleModifyUseFlag(row, useFlag) {
      updateUseFlag(row.machineTypeId).then(response => {
        this.$message({
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        ...
        if (valid) {
          ...
          createDeviceType(this.temp).then(() => {
            this.refreshList()
    ...
    updateData() {
      // this.temp.id = parseInt(Math.random() * 100) + 1024 // mock a id
      updateDeviceType(this.temp).then(() => {
        this.refreshList()
        // this.list.unshift(this.temp)
        this.dialogFormVisible = false
        this.$notify({
