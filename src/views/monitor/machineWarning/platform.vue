<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.title" clearable placeholder="请输入大屏配置名称" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    >
      <el-table-column label="大屏配置序号" prop="id" sortable="custom" align="center" :class-name="getSortClass('id')">
        <template slot-scope="{row}">
          <span>{{ row.displayPlatformId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="大屏配置编码" align="center">
        <template slot-scope="{row}">
          <span>{{ row.displayPlatformCode }}</span>
        </template>
      </el-table-column>
      <el-table-column label="大屏配置名称" width="300" align="center">
        <template slot-scope="{row}">
          <span class="link-type" @click="handleUpdate(row)">{{ row.displayPlatformName }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="停用时间" min-width="112" align="center">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="35%">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" size="mini" label-width="125px" style="width: 600px; margin-left:50px;">
        ...
        <el-form-item label="大屏配置编码" prop="displayPlatformCode">
          <el-input v-model="temp.displayPlatformCode" clearable type="text" placeholder="请输入大屏配置编码" />
        </el-form-item>
        <el-form-item label="大屏配置名称" prop="displayPlatformName">
          <el-input v-model="temp.displayPlatformName" clearable type="text" placeholder="请输入大屏配置名称" />
        </el-form-item>
...
import { fetchList, createDisplayPlatform, updateDisplayPlatform, updateUseFlag, deleteDisplayPlatform } from '@/api/displayPlatform'
import waves from '@/directive/waves' // waves directive
import { parseTime } from '@/utils'
import Pagination from '@/components/Pagination' // secondary package based on el-pagination

const displayPlatformTypeOptions = []
...
const calendarTypeKeyValue = displayPlatformTypeOptions.reduce((acc, cur) => {
  acc[cur.key] = cur.display_name
  return acc
}, {})

export default {
  name: 'DisplayPlatformTable',
  ...
  data() {
    return {
      ...
      rules: {
        ...
        displayPlatformCode: [{ required: true, message: '请填写大屏配置编码', trigger: 'blur' }],
        displayPlatformName: [{ required: true, message: '请填写大屏配置名称', trigger: 'blur' }],
  ...
  methods: {
    ...
    handleModifyUseFlag(row, useFlag) {
      updateUseFlag(row.displayPlatformId).then(response => {
        this.$message({
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        ...
        if (valid) {
          ...
          createDisplayPlatform(this.temp).then(() => {
            this.refreshList()
    ...
    updateData() {
      // this.temp.id = parseInt(Math.random() * 100) + 1024 // mock a id
      updateDisplayPlatform(this.temp).then(() => {
        this.refreshList()
        // this.list.unshift(this.temp)
        this.dialogFormVisible = false
        this.$notify({
    ...
    handleDelete(row) {
      this.$confirm('您确定要删除该数据吗?', '提示', {
        ...
        deleteDisplayPlatform(row.displayPlatformId).then(() => {
          this.refreshList()
          this.$message({
            type: 'success',
            message: '删除成功!'
